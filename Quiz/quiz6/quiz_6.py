    ...
    for y in range(dim):
        for x in range(dim):
            if grid_n[y][x] != 0:
                size = 1
                while y + size < dim:
                    cut = grid_n[y + size:y + size + 1, x - size:x + size + 1][0]
                    if len(cut) == (2 * (size + 1) - 1) and all(cut):
                        size += 1
                    else:
                        break
                if size >= 2:
                    list_size[size] += 1
    list1 = sorted(list_size.keys(), reverse=True)
    if list_size:
        print()
        print(f"For triangles pointing {fx}, we have:")
        i = 0
        while i < len(list1):
            print(f"     {list_size[list1[i]]} triangles of size {list1[i]}") if list_size[list1[i]] > 1 else print(
                f"     {list_size[list1[i]]} triangle of size {list1[i]}")
            i += 1
