    ...
    if len(year_range) != 2:
        ...
        for item in year_range:
            if len(year_range[0].strip()) != 4 or len(year_range[1].strip()) != 4:
                raise ValueError
        if not year_range[0].strip().isdigit():
            raise ValueError
        if not year_range[1].strip().isdigit():
            raise ValueError
        if int(year_range[0]) == int(year_range[1]):
            raise ValueError
        if int(year_range[0]) < 1960 or int(year_range[1]) > 2020:
            raise ValueError
except ValueError:
...
if int(year_range[0]) < 1990 or int(year_range[1]) > 2016:
    ...
    with open('./Areas/Agricultural Land/API_AG.LND.AGRI.ZS_DS2_en_csv_v2_2056230.csv', 'r') as file1:
    ...
    with open('./Areas/Forest/API_AG.LND.FRST.ZS_DS2_en_csv_v2_2125884.csv', 'r') as file2:
