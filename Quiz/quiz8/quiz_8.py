...
class Pacer:
    def __init__(self, name, Corridor):
        self.name = name
        self.dark_corridor = Corridor
        self.len = 0

    def __repr__(self):
        return f"Pacer('{self.name}', {self.dark_corridor.__repr__()})"

    def __str__(self):
        return f"{self.name} in {self.dark_corridor.__str__()}"

    def pace(self, num):
        self.len = self.len + num

    def now_here_in_dark_corridor(self):
        x, y = divmod(self.len, self.dark_corridor.length)
        print(f" {chr(11036)}" * (self.dark_corridor.length - y - 1) + f" {chr(9664)}" + f" {chr(11036)}" * y) \
            if x % 2 == 1 else \
            print(f" {chr(11036)}" * y + f" {chr(9654)}" + f" {chr(11036)}" * (self.dark_corridor.length - y - 1))
